...
% updating the weights for each of the classes separately
...
while abs(config.v_trn_best-v_prev) > .00001 &&  abs(config.v_trn_best-v_prev_prev) > .00001
    ...
    predicted_matrix_cv = zeros(NO_OF_RELNS,size(cv_cnt, 2));
    predicted_matrix_trn = zeros(NO_OF_RELNS,size(train_cnt, 2));
